...
-- Creating primary key on [Id] in table 'FileContent'
ALTER TABLE [dbo].[FileContent]
ADD CONSTRAINT [PK_FileContent]
    PRIMARY KEY CLUSTERED ([Id] ASC);
GO
...
-- Creating foreign key on [FileContentId] in table 'Flie'
ALTER TABLE [dbo].[Flie]
ADD CONSTRAINT [FK_FileContentFlie]
    FOREIGN KEY ([FileContentId])
    REFERENCES [dbo].[FileContent]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
GO

-- Creating non-clustered index for FOREIGN KEY 'FK_FileContentFlie'
CREATE INDEX [IX_FK_FileContentFlie]
ON [dbo].[Flie]
    ([FileContentId]);
GO
